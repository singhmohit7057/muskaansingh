      ...
      <section className="py-20 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            >
              <img src="/muskaan/Muskaan_Singh_Founder.webp" alt="Muskaan Singh, founder of Label Muskaan Singh" className="w-full h-full object-contain p-8" loading="lazy" width="400" height="400" />
            </div>
      ...
      <section className="py-20 px-6 md:px-12">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            <div className="w-8 h-0.5 rounded-full mb-6 bg-[#4C40C0]" />
            <p className="text-[#4B5563] leading-relaxed mb-4 font-medium">
          ...
          >
            ...
            <p className="text-[#4B5563] leading-relaxed mb-4 font-medium">
              <em className="not-italic font-bold" style={{ color: "#006060" }}>Ibtida</em> — a new beginning. The daily wear sub-brand under Label Muskaan Singh.
            </p>
            <p className="text-[#4B5563] leading-relaxed mb-8 text-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-8">
            {celebrities.map((celeb, i) => (
              ...
              >
                ...
                <p className="text-xs font-semibold text-[#3D4852] text-center group-hover:text-[#4C40C0] transition-colors duration-300 leading-tight">
                  {celeb.name}
                </p>
              </motion.a>
            ))}
            ...
            <div className="flex flex-col gap-2 mb-10 md:mb-0">
              {instagramAccounts.map((acc) => (
                <a key={acc.handle} href={acc.link} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 text-sm font-medium text-[#4B5563] hover:text-[#4C40C0] transition-colors group">
                  <AtSign size={12} />
                  {acc.handle.slice(1)}
          ...
          {[
            ...
            <div key={account} className="mb-8">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-xs font-semibold text-[#4B5563] hover:text-[#4C40C0] transition-colors mb-3"
              >
                <AtSign size={11} />
                {account}
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                {instagramPosts.filter(p => p.account === account).map((post, i) => (
                  <motion.a
                    ...
                    whileHover={{ y: -3 }}
                  >
                    <img src={post.image} alt={`Instagram post by @${post.account}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" width="300" height="375" />
                    <div className="absolute inset-0 bg-[#4C40C0]/0 group-hover:bg-[#4C40C0]/20 transition-all duration-300 flex items-center justify-center">
